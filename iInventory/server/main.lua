...
RegisterServerEvent("iInventory:sendShopItems")
AddEventHandler("iInventory:sendShopItems", function(source, itemList)
	itemShopList = itemList
end)

ESX.RegisterServerCallback("iInventory:getShopItems", function(source, cb, shoptype)
	itemShopList = {}
...
ESX.RegisterServerCallback("iInventory:getCustomShopItems", function(source, cb, shoptype, customInventory)
	itemShopList = {}
	local itemResult = MySQL.Sync.fetchAll('SELECT * FROM items')
	local itemInformation = {}
...
RegisterNetEvent("iInventory:SellItemToPlayer")
AddEventHandler("iInventory:SellItemToPlayer",function(source, type, item, count)
            ...
            for i = 1, #list, 1 do
				if list[i].name == item then
					...
					if xPlayer.getMoney() >= totalPrice then
						...
						TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'success', text = 'vous avez acheté '..count.." "..list[i].label })
					else
						TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'error', text = 'Vous n\'avez pas assez d\'argent!' })
					end
				end
            end
        else
            TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'error', text = 'Vous n\'avez pas assez d\'espace dans votre inventaire!' })
        end
	end
				...
				if list[i].name == item then
					...
					if not targetWeapon then
						...
						if xPlayer.getMoney() >= totalPrice then
							...
							TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'success', text = 'vous avez acheté un '..list[i].label })
						else
							TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'error', text = 'Vous n\'avez pas assez d\'argent!' })
						end
					else
						TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'error', text = 'Vous possédez déjà cette arme!' })
					end
				end
            end
        else
		...
		for i = 1, #list, 1 do
			if list[i].name == item then
				...
				if targetWeapon then
					...
					if xPlayer.getMoney() >= totalPrice then
						xPlayer.removeMoney(totalPrice)
						TriggerClientEvent("iInventory:AddAmmoToWeapon", source, list[i].weaponhash, ammo)
						TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'success', text = 'vous avez acheté '..count.." "..list[i].label })
					else
						TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'error', text = 'Vous n\'avez pas assez d\'argent!' })
					end
				else
					TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'error', text = 'Vous ne possédez pas d\'arme pour ce type de munitions!' })
				end
            end
        end
    end
...
function GetLicenses(source)
    TriggerEvent('esx_license:getLicenses', source, function (licenses)
        TriggerClientEvent('iInventory:GetLicenses', source, licenses)
    end)
end

RegisterServerEvent('iInventory:buyLicense')
AddEventHandler('iInventory:buyLicense', function ()
	local _source = source
	local xPlayer = ESX.GetPlayerFromId(source)
	if xPlayer.get('money') >= Config.LicensePrice then
		xPlayer.removeMoney(Config.LicensePrice)
		TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'success', text = 'Vous avez reçu un permis de port d\'armes.' })
