...
function OpenShopInv(shoptype)
    ...
    ESX.TriggerServerCallback("iInventory:getShopItems", function(shopInv)
...
RegisterNetEvent("iInventory:OpenCustomShopInventory")
AddEventHandler("iInventory:OpenCustomShopInventory", function(type, shopinventory)
    text = "shop"
    data = {text = text}
    inventory = {}

    ESX.TriggerServerCallback("iInventory:getCustomShopItems", function(shopInv)
        for i = 1, #shopInv, 1 do
            table.insert(inventory, shopInv[i])
        end
...
RegisterNUICallback("TakeFromShop", function(data, cb)
        ...
        if type(data.number) == "number" and math.floor(data.number) == data.number then
            TriggerServerEvent("iInventory:SellItemToPlayer", GetPlayerServerId(PlayerId()), data.item.type, data.item.name, tonumber(data.number))
        end
...
RegisterNetEvent("iInventory:AddAmmoToWeapon")
AddEventHandler("iInventory:AddAmmoToWeapon", function(hash, amount)
...
RegisterNetEvent('iInventory:GetLicenses')
AddEventHandler('iInventory:GetLicenses', function (licenses)
    ...
    ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'shop_license',{
      ...
      function (data, menu)
        if data.current.value == 'yes' then
            TriggerServerEvent('iInventory:buyLicense')
        end
        menu.close()
    end,
    function (data, menu)
